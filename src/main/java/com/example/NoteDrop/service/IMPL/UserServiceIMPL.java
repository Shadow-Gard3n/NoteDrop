...
public class UserServiceIMPL implements UserService, UserDetailsService {
    ...
    public void followUser(String followerUsername, String followedUsername) {
        ...
        // Verify users exist
        if (!userRepo.findByUsername(followerUsername).isPresent()) {
            throw new IllegalArgumentException("Follower user not found.");
        }
        if (!userRepo.findByUsername(followedUsername).isPresent()) {
            throw new IllegalArgumentException("User to follow not found.");
        }

        boolean alreadyFollowing = followRepo.existsByFollowerUsernameAndFollowedUsername(followerUsername, followedUsername);
        if (alreadyFollowing) {
    ...
    public List<String> getFollowedUsernames(String username) {
        List<Follow> follows = followRepo.findByFollowerUsername(username);
